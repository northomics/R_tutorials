...
## loading ALL tidyverse packages at once because we'll be using many of them
library(tidyverse)
...
Fix this so that it's just a solution and doesnt print!!!
```{r penguindots, exercise=T}
ggplot(data=penguins, aes(x=????, y=????, colour=?????)) + 
  geom_????()
```
```{r penguinsdots-solution}
